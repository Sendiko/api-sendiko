...
export default class JwtToken {
  ...
  constructor({ secret, expires }: JwtTokenParams) {
    this._secret = secret
    this._expires = validate.ms(expires)
  ...
  verify(token: string) {
    try {
      ...
      if (error instanceof jwt.TokenExpiredError) {
        return {
          data: null,
          message: `unauthorized, token expired ${error.message || error}`,
        }
      }
      ...
      if (error instanceof jwt.JsonWebTokenError) {
        return {
          data: null,
          message: `unauthorized, invalid token ${error.message || error}`,
        }
      }
      ...
      if (error instanceof jwt.NotBeforeError) {
        return {
          data: null,
          message: `unauthorized, token not before ${error.message || error}`,
        }
      }
